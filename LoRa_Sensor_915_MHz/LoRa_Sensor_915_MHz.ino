...
void onReceive(){
  ...
  if((sbjMsg == "CONFIG" && cfgSnr == false) || (sbjMsg == "RECEIV" && recAddr == snrAddr)){
    ...
    uint32_t ETtr = recMsg.substring(7,aux1).toInt();
...
snrNum - uint8_t - Number of sensors
...
ETtr - uint32_t - Epoch time transmitted by the gateway
...
RTItr - uint32_t - Time interval between readings transmitted by the gateway
sum - uint32_t - Accumulator
TTI - uint32_t - Transmission time interval (milliseconds)
TTItr - uint32_t - Time interval between transmissions transmitted by the gateway
...
getBandgap() - float - Arduino's bandgap voltage
