...
const uint32_t HZ = 920E6;
const bool crcEna = true;

const int8_t LTZ = -3;
const uint8_t dsTime = 1;
...
String vetMsg;
String sndMsg;
uint32_t RTI = 1.0 * 60 * 60 * 1000;
uint32_t TTI = 4.5 * RTI;
uint32_t aTime = 15 * 60 * 1000;
uint32_t eTime = 946758403;
...
WiFiUDP ntpUDP;
NTPClient timeClient(ntpUDP,"pool.ntp.org",LTZ * 3600,dsTime * 3600);
...
void drawFrame1(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t HDP, int16_t VDP) {
  ...
  display->drawString(cenXol + HDP,cenYol + VDP + 12,"Sistema LoRa gateway");
}
...
void sendCFG(uint8_t snrAddr){
  ...
  if(snrAddr == netAddr){
    sndMsg = String("CONFIG:" + String(eTime) + ";" + String(RTI) + ";" + String(TTI));
  }else{
    sndMsg = String("RECEIV:" + String(eTime) + ";" + String(RTI) + ";" + String(TTI));
  }
...
RSSI - String - Broadcast RSSI value
SDDisp - String - Available space on the SD Card shown on the display
sndMsg - String - Message to be sent
SNR - String - Transmission SNR value
...
LTZ - int8_t - Local time zone
HDP - int16_t - Horizontal display positioner
VDP - int16_t - Vertical display positioner
lTime - int32_t - Accumulated time since the last of the correct hour
cenXol - uint8_t - Central X coordinate of the display
cenYol - uint8_t - Central Y coordinate of the blue band on the display
...
dsTime - uint8_t - Update interval (hours)
FC - uint8_t - Number of frames
gtwAddr - uint8_t - Address of this LoRa gateway device
Hol - uint8_t - Vertical screen size in pixels
netAddr - uint8_t - Generic sensor address
...
TP - uint8_t - LoRa TX power: 2-22
typSD - uint8_t - SD Card type
Wol - uint8_t - Horizontal screen size in pixels
incLen - uint16_t - Size of received message
RTB - uint16_t - Time remaining until display update
aTime - uint32_t - Time delay in sending the correct time (milliseconds)
BW - uint32_t - LoRa signal bandwidth: 125E3, 250E3, 500E3
eTime - uint32_t - Epoch Time (seconds)
HZ - uint32_t - LoRa Frequency (Hz)
LST - uint32_t - Timestamp of the last time-correct message sent
LWF - uint32_t - Time since last WiFi update
...
spcSD - uint64_t - Amount of available memory on SD Card (MB)
useSD - uint64_t - Amount of memory used on SD Card (MB)
*/
